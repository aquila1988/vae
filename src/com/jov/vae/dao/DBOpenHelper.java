...
public class DBOpenHelper extends SQLiteOpenHelper {
	...
	private static final String DB_NAME = "vaecn.db";
	/**
	 * version
	 * */
	...
	private static final String CREATE_TABLE_TEXT = "create table IF NOT EXISTS "
			+ TABLE_NAME_TEXT
			+ "(tid integer primary key autoincrement,content varchar(6000),imgurl varchar(500),onsite varchar(100),uptime varchar(20))";
...
/*
	...
	public boolean hasTextDate(String content) {
		if (db == null||!db.isOpen()) {
			db = this.getReadableDatabase();
		}
		Cursor cursor = null;
		cursor = db.rawQuery("select * from " + TABLE_NAME_TEXT
				+ " where content=?  limit 1  ", new String[] { content });
		boolean has = false;
	...
	public void insertText(TextBean bean) {
		...
		db.execSQL(sql, new String[] { bean.getContent(), bean.getImgurl(),
				bean.getOnsite(),bean.getUptime() });
		db.close();
	}
	...
	public List<TextBean> getTexts(int offset) {
		...
		cursor = db.rawQuery(
				"select * from " + TABLE_NAME_TEXT
						+ " order by tid,uptime desc  limit 10 offset "
		...
		while (cursor != null && cursor.moveToNext()) {
			...
			obj.setOnsite(cursor.getString(cursor.getColumnIndex("onsite")));
			obj.setUptime(cursor.getString(cursor.getColumnIndex("uptime")));
			list.add(obj);
		}
		cursor.close();
		db.close();
	...
	public TextBean getText(int tid) {
		...
		while (cursor != null && cursor.moveToNext()) {
			obj = new TextBean();
			obj.setTid(cursor.getInt(cursor.getColumnIndex("tid")));
			obj.setImgurl(cursor.getString(cursor.getColumnIndex("image")));
			obj.setContent(cursor.getString(cursor.getColumnIndex("content")));
			obj.setOnsite(cursor.getString(cursor.getColumnIndex("onsite")));
			obj.setUptime(cursor.getString(cursor.getColumnIndex("uptime")));
		}
